...
class RdkitGridFeaturizer(ComplexFeaturizer):
  ...
  def __init__(self,
    ...
    **kwargs: dict, optional
    ...
    self.splif_featurizer = SplifFingerprint(
        ...
        radius=self.ecfp_degree, size=2**self.splif_power)
  ...
  def _compute_feature(self, feature_name, prot_xyz, prot_rdk,
    ...
    if feature_name == 'hbond_count':
      return self.hbond_counter._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    ...
    if feature_name == 'salt_bridge':
      return self.salt_bridge_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    if feature_name == 'charge':
      return self.charge_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    if feature_name == 'hbond':
      return self.hbond_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    if feature_name == 'pi_stack':
      return self.pi_stack_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    if feature_name == 'cation_pi':
      return self.cation_pi_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
