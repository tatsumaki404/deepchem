...
#TODO(rbharath): Find references for SPLIF, SYBYL and add docstrings
...
sybyl_types = [
    "C3", "C2", "C1", "Cac", "Car", "N3", "N3+", "Npl", "N2",
    "N1", "Ng+", "Nox", "Nar", "Ntr", "Nam", "Npl3", "N4", "O3",
    "O-", "O2", "O.co2", "O.spc", "O.t3p", "S3", "S3+", "S2",
    "So2", "Sox" "Sac" "SO", "P3", "P", "P3+", "F", "Cl", "Br",
    "I"
]
...
VOXEL_FEATURES = [
    'ecfp', 'splif', 'sybyl', 'salt_bridge', 'charge',
...
class RdkitGridFeaturizer(ComplexFeaturizer):
  ...
  def __init__(self,
    ...
    **kwargs: dict, optional
    ...
    self.ecfp_featurizer = CircularFingerprint(radius=self.ecfp_degree, size=2**self.ecfp_power)
    self.contact_featurizer = ContactCircularFingerprint(cutoff=self.cutoffs['ecfp_cutoff'], radius=self.ecfp_degree, size=2**self.ecfp_power)
    self.contact_voxelizer = ContactCircularVoxelizer(cutoff=self.cutoffs['ecfp_cutoff'], radius=self.ecfp_degree, size=2**self.ecfp_power, box_width=self.box_width, voxel_width=self.voxel_width)
    self.splif_featurizer = SplifFingerprint(contact_bins=self.cutoffs['splif_contact_bins'], radius=self.ecfp_degree, size=2**self.splif_power)
    self.splif_voxelizer = SplifVoxelizer(contact_bins=self.cutoffs['splif_contact_bins'], radius=self.ecfp_degree, size=2**self.splif_power, box_width=self.box_width, voxel_width=self.voxel_width)

    # parse provided feature types
    for feature_type in feature_types:
      if self.sanitize is False and feature_type in require_sanitized:
        logger.warning('sanitize is set to False, %s feature will be ignored' % feature_type)
        continue
      ...
      if feature_type in FLAT_FEATURES:
        ...
        if self.flatten is False:
          logger.warning('%s feature is used, output will be flattened' % feature_type)
          self.flatten = True
  ...
  def _compute_feature(self, feature_name, prot_xyz, prot_rdk,
                       lig_xyz, lig_rdk, distances):
    if feature_name == 'ecfp_ligand':
      return [self.ecfp_featurizer(lig_rdk)]
    if feature_name == 'ecfp_hashed':
      return self.contact_featurizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    if feature_name == 'splif_hashed':
      return self.splif_featurizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    ...
    if feature_name == 'ecfp':
      return self.contact_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
    if feature_name == 'splif':
      return self.splif_voxelizer._featurize_complex(
        (lig_xyz, lig_rdk), (prot_xyz, prot_rdk))
