...
def _compute_neighbor_list(coords,
                           neighbor_cutoff,
                           max_num_neighbors,
                           periodic_box_size):
...
class AtomicConvFeaturizer(ComplexFeaturizer):
  ...
  def _featurize_complex(self, molecular_complex):
    ...
    if self.strip_hydrogens:
      fragments = [rdkit_util.strip_hydrogens(frag[0], frag[1]) for frag in fragments]
      system_coords, system_mol = rdkit_util.strip_hydrogens(system_coords, system_mol)

    try:
      frag_inputs = [self.featurize_mol(frag[0], frag[1], frag_num_atoms) for (frag, frag_num_atoms) in zip(fragments, self.frag_num_atoms)]

      system_outputs = self.featurize_mol(
...
class ComplexNeighborListFragmentAtomicCoordinates(AtomicConvFeaturizer):
  def __init__(self, *args, **kwargs):
    ...
    super(ComplexNeighborListFragmentAtomicCoordinates, self).__init__(*args, **kwargs)
